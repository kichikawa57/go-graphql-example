...
func (tq *TweetQuery) QueryGoods() *GoodQuery {
	query := &GoodQuery{config: tq.config}
...
func (tq *TweetQuery) QueryComments() *CommentQuery {
...
func (tq *TweetQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (tq *TweetQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (tq *TweetQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (tq *TweetQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (tq *TweetQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (tq *TweetQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TweetQuery) WithGoods(opts ...func(*GoodQuery)) *TweetQuery {
	query := &GoodQuery{config: tq.config}
...
func (tq *TweetQuery) WithComments(opts ...func(*CommentQuery)) *TweetQuery {
	query := &CommentQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TweetQuery) sqlAll(ctx context.Context) ([]*Tweet, error) {
	...
	if query := tq.withGoods; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Tweet)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Goods = []*Good{}
		}
		query.withFKs = true
		query.Where(predicate.Good(func(s *sql.Selector) {
			s.Where(sql.InValues(tweet.GoodsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.tweet_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tweet_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
	...
	if query := tq.withComments; query != nil {
		...
		query.Where(predicate.Comment(func(s *sql.Selector) {
			s.Where(sql.InValues(tweet.CommentsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
